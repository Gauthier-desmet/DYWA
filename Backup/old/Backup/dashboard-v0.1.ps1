...
$filer1 = "lciserv2.iemn.fr" # filer n°1
...
$FormLogin = New-UDAuthenticationMethod -Endpoint {
    ...
    ##Bad Pass
    New-UDAuthenticationResult -ErrorMessage "Identifiants incorrects / seuls la direction et le SISR ont un droit d'accès"
}
...
} -AuthorizedRole @("Admin") #fin page Accueil
...
New-UDCard -Title "Date et Heure Forêt IEMN" -Content {
    ...
    New-UDRow -Columns{
        ...
        New-UDCounter -BackgroundColor "cornflowerblue" -FontColor "#fff" -Icon cloud -Title "$spcountsite Sites sur l'intranet sont UP" -TextSize 2 -AutoRefresh -RefreshInterval 60 -Endpoint {$spcountsite}
    }
...
} -AuthorizedRole @("User", "Admin")  # fin page status
...
Select-Object -Property IPAddress,MACAddress,DNSHostName,DNSServerSearchOrder | Out-UDGridData
 ...
 } -AuthorizedRole @("Admin") # fin page ADS
...
####### fin liste de url publiques
 } -AuthorizedRole @("Admin") # fin page SharePoint
...
####### fin udlayout
 ...
 } -AuthorizedRole @("Admin") # fin monitor
...
New-UdGrid -BackgroundColor '#333' -Title "Certificats" -FontColor '#fff' -Headers @("Rôle", "Etât", "Expire le") -Properties @("Role", "Level", "ExpiresOn") -AutoRefresh -RefreshInterval 600 -Endpoint {
 ...
 } -AuthorizedRole @("Admin") # fin page RDS
...
New-UDCounter -Title "Nombre de produits supportés" -Endpoint {$productwsus}
 } -AuthorizedRole @("Admin") # fin page WSUS
...
New-UdGrid -Title "Historique des échecs de téléchargements TFTP" -Headers @("Date", "Nom") -Properties @("TimeCreated", "Message") -AutoRefresh -RefreshInterval 60 -Endpoint {
 ...
 } -AuthorizedRole @("Admin") # fin page WDS
...
Show-UDToast -Icon trash -IconColor "white" -MessageColor "#ec7331" -BackgroundColor "#ccc" -Duration 10000 -Message "On enlève les âneries et on recommence !"  
 ...
 } -AuthorizedRole @("User", "Admin") # fin page WPS
...
Select-Object -Property IPAddress,MACAddress,DNSHostName,DNSServerSearchOrder | Out-UDGridData
 ...
 } -AuthorizedRole @("Admin") # fin page SafeQ
...
} -AuthorizedRole @("Admin") #fin de page wapt
...
} -AuthorizedRole @("Admin") #fin de page kasc
...
} -AuthorizedRole @("Admin") #fin de page urbackup
...
} -AuthorizedRole @("Admin") #fin page bitlocker
...
New-UDRow -Columns{
    New-UDColumn -Size 2 -Content{
        New-UDCounter -Title "$ADcomptotal postes sous AD" -BackgroundColor "cornflowerblue" -Icon laptop -AutoRefresh -RefreshInterval 60 -Endpoint {$ADcomptotal}
    }
    New-UDColumn -Size 2 -Content{
        New-UDCounter -Title "Dont $ADcompwinxp postes Windows xp" -BackgroundColor "#e95555" -Icon laptop -AutoRefresh -RefreshInterval 60 -Endpoint {$ADcompwinxp}
    }
    New-UDColumn -Size 2 -Content{
    New-UDCounter -Title "Dont $ADcompwin7 postes Windows 7" -BackgroundColor "#ec7331" -Icon laptop -AutoRefresh -RefreshInterval 60 -Endpoint {$ADcompwin7}
    }
    New-UDColumn -Size 2 -Content{
        New-UDCounter -Title "Dont $ADcompwin8 postes Windows 8 et 8.1" -FontColor "#333" -BackgroundColor "#ecb531" -Icon laptop -AutoRefresh -RefreshInterval 60 -Endpoint {$ADcompwin8}
    }
    New-UDColumn -Size 2 -Content{
        New-UDCounter -Title "Dont $ADcompwin10 postes Windows 10" -FontColor "#333" -BackgroundColor "lightgreen" -Icon laptop -AutoRefresh -RefreshInterval 60 -Endpoint {$ADcompwin10}
    }
    New-UDColumn -Size 2 -Content{
        New-UDCounter -Title "Dont $ADcompwinserv postes Windows Server" -FontColor "#333" -BackgroundColor "lightgreen" -Icon server -AutoRefresh -RefreshInterval 60 -Endpoint {$ADcompwinserv}
    }
}
...
New-UdGrid -Title "Postes clients sous AD" -Headers @("Noms DNS AD", "SamAccountName") -Properties @("DNSHostName", "SamAccountName") -AutoRefresh -RefreshInterval 60 -Endpoint {
       Get-ADComputer  -Filter * -SearchBase "CN=Computers,DC=iemn,DC=fr" -Properties * | Select DNSHostName,SamAccountName | Out-UDGridData
...
} -AuthorizedRole @("Admin") # fin page ad infos 
...
New-UdGrid -Title "LciServ VSAN Hyper-V" -Headers @("Nom", "State") -Properties @("Name", "State") -FontColor '#fff' -AutoRefresh -RefreshInterval 60 -Endpoint {
...
} -AuthorizedRole @("User", "Admin") #fin page cluster
...
Get-SmbShare -CimSession $cim1 | where {$_.ScopeName -like "LCISERV2"} | Select * | Out-UDGridData 
}#fin udgrid
}#fin udcolumn
New-UDColumn -Size 5 -Content{
New-UdChart -Title "Espace disques" -Type Bar -AutoRefresh -Endpoint {
...
} -AuthorizedRole @("Admin") #fin page filers

# Lancement de la DashBoard
$iemn_dashboard = New-UDDashboard -Title "$titre" -NavBarColor $bgcolor -BackgroundColor $bgcolor -Footer $Footer -Theme $Theme -Stylesheets $css -LoginPage $LoginPage -Pages @($page_Accueil, $page_status, $page_Hyperv, $page_infoclustmonitor, $page_ActiveDirectory, $page_infoADmonitor, $page_rds, $page_intranet, $page_WSUS, $page_WDS, $page_WPS, $page_Ysoft, $page_CA, $page_wapt, $page_kasc, $page_filers, $page_urbackup, $page_Bitlocker)
Start-UDDashboard -Port 8080 -Dashboard $iemn_dashboard -Certificate $cert -AutoReload # Je lance la tâche sur le port 8080 avec le nom de variable ci-dessus avec l'option d'actualisation des datas.
